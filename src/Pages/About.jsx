...
export default function About() {
  ...
  return (
    <div>
      <h2 className="text-5xl font-bold text-center text-gray-700 pt-24">
      ...
      {aboutSections.map((section, index) => (
        <div
          key={index}
          className="flex flex-row justify-center items-center pt-12 gap-24"
        >
          {section.imgFirst && (
            <img
              src={section.img}
          ...
          <aside className="max-w-2xl">
            <h2 className="text-slate-700 text-2xl font-semibold mb-5">
              {section.title}
            </h2>
            <p className="font-light leading-7">{section.text}</p>
            <button className="bg-orange-500 px-8 py-3 rounded-full w-40 text-white shadow-2xl hover:shadow-orange-400 mt-10">
              Know more..
            </button>
          </aside>
          {!section.imgFirst && (
